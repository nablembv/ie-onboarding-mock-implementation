...
class Base {
  ...
  private function _getPageNumber() {
    ...
    elseif (isset($this->config['page_params']['base_url_bvstate'])) {
    ...
    elseif (isset($this->config['bv_page_data']['bvrrp'])) {
      $bvparam = $this->config['bv_page_data']['bvrrp'];
    }
    elseif (isset($this->config['bv_page_data']['bvqap'])) {
      $bvparam = $this->config['bv_page_data']['bvqap'];
    }
    elseif (isset($this->config['bv_page_data']['bvsyp'])) {
  ...
  private function _replaceTokens($content) {
    ...
    elseif (mb_ereg('_escaped_fragment_=.+$', $this->config['base_url'])) {
      $page_url_query_prefix = '%26';
    }
    // Otherwise we're back to thinking about query strings.
    elseif (!mb_ereg('[\?&]$', $this->config['base_url'])) {
